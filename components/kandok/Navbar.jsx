    ...
    <nav className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 shadow-sm sticky top-0 z-50 transition-colors duration-300">
      ...
      {mobileOpen && (
        <div className="md:hidden px-6 pb-6 space-y-4 text-sm font-medium border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-lg">
          ...
          {(isSeller || isBem || isKantek || isKandok || isKantel || isKansip || isBerkah || isKantintn || isTaniamart) && <hr className="my-4 border-gray-100 dark:border-gray-800" />}
          ...
          <div className="relative search-container">
            <form onSubmit={handleSearch} className="flex border border-gray-200 dark:border-gray-700 hover:border-[#479c26] dark:hover:border-green-500 rounded-full px-4 py-2 items-center mt-4 transition-colors duration-300 group">
              <input
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                onFocus={() => searchResults.length > 0 && setShowResults(true)}
                placeholder="Cari makanan..."
                className="text-sm outline-none w-full text-gray-600 dark:text-gray-300 placeholder-gray-400 dark:placeholder-gray-500 bg-transparent"
              />
              <button type="submit" className="group-hover:scale-110 transition-transform duration-200">
                <Image src={assets.search_icon} alt="search" className="w-4 h-4 dark:invert" />
              </button>
            ...
            {showResults && searchResults.length > 0 && (
              <div className="absolute left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden z-50 border border-gray-100 dark:border-gray-700">
                <ul>
                  {searchResults.map((product, index) => (
                    <li 
                      key={index} 
          ...
          <div className="mt-4">
            {user ? (
              ...
              <button 
                onClick={openSignIn} 
                className="flex items-center gap-2 w-full justify-center bg-white dark:bg-gray-900 text-[#479c26] border border-[#479c26] hover:bg-[#479c26]/10 transition-all duration-300 px-4 py-2 rounded-full shadow-sm hover:shadow-md"
              >
                <Image src={assets.user_icon} alt="user icon" className="dark:invert" />
                <span>Login</span>
              </button>
