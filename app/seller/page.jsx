...
const AddProduct = () => {
  ...
  const [kantin, setKantin] = useState('Kantin Teknik'); // Menambahkan state untuk kantin
  ...
  return (
    <div className="flex-1 min-h-screen flex flex-col justify-between">
      <form onSubmit={handleSubmit} className="md:p-10 p-4 space-y-5 max-w-lg">
        <div>
          <p className="text-base font-medium">Foto Produk</p>
        ...
        <div className="flex items-center gap-5 flex-wrap">
          ...
          <div className="flex flex-col gap-1 w-40">
            ...
            <select
              id="kantin"
              className="outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500/40"
              onChange={(e) => setKantin(e.target.value)}
              value={kantin}
              required
            >
              {kantinOptions.map((kantinOption) => (
                <option key={kantinOption} value={kantinOption}>
                  {kantinOption}
                </option>
              ))}
            </select>
